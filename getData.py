...
def find_all_sim_cards(client):
    ...
    if get_mysims_response.status_code == 200:
        ...
        raise RuntimeError('When reading sim cards list response was not 200, but ',
...
def extract_csv(client, start_date, end_date, sim_card_to_extract):
    ...
    if csv_response.headers['Content-Type'] != 'text/csv':
        ...
        out_csv_file_name = create_csv_file_name(end_date, sim_card_to_extract, start_date)
        with open(out_csv_file_name, 'w') as out_csv_file:
            out_csv_file.write(csv_response.text)
            print("saved to file " + out_csv_file_name)


def create_csv_file_name(end_date, sim_card_to_extract, start_date):
    return sim_card_to_extract['phone_no'].replace(" ", "-") + "_" + str(
...
beginning_of_the_month = beginning_of_the_month()
...
parser.add_argument('--start_date', help='start date to read history or 1st day of month will be used',
                    default=beginning_of_the_month, type=lambda d: datetime.datetime.strptime(d, DATE_FORMAT))
